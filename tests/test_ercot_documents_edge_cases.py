...
class TestDocumentsCoverage:
    ...
    def test_get_documents_exceptions(self):
        ...
        with patch("httpx.Client") as mock_client:
            mock_client.return_value.__enter__.return_value.get.side_effect = Exception("Net error")
        ...
        with patch("httpx.Client") as mock_client:
            mock_resp = MagicMock()
            mock_resp.json.return_value = {"ListDocsByRptTypeRes": {"DocumentList": [{"Bad": "Data"}]}}
            mock_client.return_value.__enter__.return_value.get.return_value = mock_resp
            
            with patch("tinygrid.ercot.documents.Document.from_json", side_effect=Exception("Parse error")):
                docs = mixin._get_documents(123)
                assert docs == []
    ...
    def test_read_doc_coverage(self):
        ...
        with patch("httpx.Client") as mock_client:
            mock_client.return_value.__enter__.return_value.get.side_effect = Exception("Download failed")
        ...
        with patch("httpx.Client") as mock_client:
            ...
            with patch("zipfile.ZipFile") as mock_zip:
                ...
                zf_mock.open.return_value.__enter__.return_value.read.return_value = b"garbage"
                ...
                with patch("pandas.read_csv", side_effect=Exception("CSV Fail")):
                    with patch("pandas.read_excel", side_effect=Exception("Excel Fail")):
                        df = mixin.read_doc(doc)
    ...
    def test_get_mapping_coverage(self):
        ...
        # Download fail
        with patch.object(mixin, "_get_document", return_value=Document("url", pd.Timestamp.now(), "id", "name", "friendly")):
            with patch("httpx.Client") as mock_client:
