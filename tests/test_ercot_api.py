...
from tinygrid.constants.ercot import LocationType, Market
...
class TestERCOTAPIMixin(ERCOTAPIMixin):
    ...
    get_shadow_prices_bound_transmission_constraint = MagicMock(
...
class TestAPICoverage:
    def test_get_spp_historical(self):
        ...
        assert (
            mixin._archive_mock.fetch_historical.call_args[1]["endpoint"]
    ...
    def test_get_lmp_historical(self):
        ...
        mixin.get_lmp(
            market=Market.REAL_TIME_SCED, location_type=LocationType.ELECTRICAL_BUS
        )
        assert (
            mixin._archive_mock.fetch_historical.call_args[1]["endpoint"]
        ...
        mixin.get_lmp(
            market=Market.REAL_TIME_SCED, location_type=LocationType.RESOURCE_NODE
        )
        assert (
            mixin._archive_mock.fetch_historical.call_args[1]["endpoint"]
